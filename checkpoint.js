...
let friend = amigos.filter(function(el){
...
function pluck(array, propiedad) {
  ...
  let arrayProp = array.map(function(el){
    return el[propiedad];
  })
...
function crearClasePersona() {
  ...
  class Persona {
    ...
    getFriends() {
      ...
      let friendNames = this.amigos.map(function(el){
        return el.nombre;
      });
      return friendNames;
    }
    ...
    getPromedioEdad() {
      ...
      let sumaEdades = this.amigos.reduce(function(acc,el){
        return acc + el.edad;
      },0);
